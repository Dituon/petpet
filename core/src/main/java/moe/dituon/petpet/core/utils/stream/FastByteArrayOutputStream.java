 ...
 * by a {@link ArrayDeque} of {@code byte[]} buffers instead of one
...
public class FastByteArrayOutputStream extends OutputStream {
    ...
    public FastByteArrayOutputStream(int initialBlockSize) {
        isTrue(initialBlockSize > 0, "Initial block size must be greater than 0");
    ...
    public void resize(int targetCapacity) {
        isTrue(targetCapacity >= size(), "New capacity must not be smaller than current size");
    ...
    protected static void isTrue(boolean expression, String message) {
        if (!expression) {
